...
/*const bingoSelectionExample = [7, 4, 9, 5, 11, 17, 23, 2, 0, 14, 21, 24, 10, 16, 13, 6, 15, 25, 12, 22, 18, 20, 8, 19, 3, 26, 1];

const bingoBoardExample = [
...
let gridLength = bingoBoardCopy[0].length;
let i;
let idWinningGrids = [];
let lastNumberDrawn;
let lastWinningGrid = -1;
let numberDrawn;
let selectionLength = bingoSelection.length;

function checkWinningGrids(board) {
    let countNumbersColumn = 0;
    let countNumbersRow = 0;
    let gridDimentions = 5;
    let j;
    let k;
    let l;
    let m;

    board.forEach((row, idRow) => {
        k = 0;
        m = 0;
        while (k < gridLength) {
            for (j = k; j < (gridDimentions + k); j++) {
                if (row[j] === -1) {
                    countNumbersRow++;
                }
            }
            if (countNumbersRow === 5) {
                idWinningGrids.push(idRow);
            }
            k += 5;
            countNumbersRow = 0;
        }
        while (m < gridDimentions) {
            for (l = m; l < gridLength; l+=5) {
                if (row[l] === -1) {
                    countNumbersColumn++;
                }
            }
            if (countNumbersColumn === 5) {
                idWinningGrids.push(idRow);
            }
            m++;
            countNumbersColumn = 0;
        }
    });
}

function drawGrids(grid, nb) {
...
for (i = 0; i < selectionLength; i++) {
    numberDrawn = bingoSelection[i];
    drawGrids(bingoBoardCopy, numberDrawn);
    checkWinningGrids(bingoBoardCopy);
    if (idWinningGrids.length >= 1) {
        lastWinningGrid = bingoBoardCopy[idWinningGrids.slice(-1)];
        lastNumberDrawn = numberDrawn;
        idWinningGrids.forEach((id) => {
            delete bingoBoardCopy[id];
            idWinningGrids = [];
        });
    }
}
console.log(finalScore(lastNumberDrawn, lastWinningGrid));
